...
try:
    ...
    ronaldo = soup.find(
        'div',
        class_='StatsBlock-module--statsBlock--4b2a6 StatsBlock-module--RonaldoStatsBlock--f266e relative text-center w-1/2'
    )
    goal_counter = ronaldo.find('li', class_='StatsBlock-module--goals--df119') if ronaldo else None
    ...
    if os.path.exists(GOAL_COUNT_FILE):
        with open(GOAL_COUNT_FILE, 'r') as f:
            previous_goals = int(f.read().strip() or 0)
...
filled_length = int((previous_goals / thousand) * bar_length) if thousand else 0
...
def _is_payment_required(exc: Exception) -> bool:
    # Tweepy HTTPException often includes a Response with status_code
    ...
    return (
        ...
        or "does not have any credits" in msg
        or "does not have any credit" in msg
    )
...
if goals > 0:
    try:
        ...
        if _is_payment_required(e):
            print("X API credits exhausted (402 Payment Required). Skipping tweet without failing the workflow.")
            sys.exit(0)
        raise
